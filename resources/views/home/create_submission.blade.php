...
<h1>
    @if($isClaim)
        New Claim
    @else
        New Activity/Prompt Submission
    @endif
</h1>
...
@else
    {!! Form::open(['url' => $isClaim ? 'claims/new' : 'submissions/new', 'id' => 'submissionForm']) !!}
        ...
        @endif
        <div class="form-group">
            {!! Form::label('url', $isClaim ? 'URL Link (Required)' : 'Submission URL') !!}
            @if($isClaim)
            ...
            @else
                ...
                <br><small style="padding-left:10px">Enter the URL of your submission from the <a href="/gallery">gallery</a>, story from the <a href="/forum">forum</a> or other hosting service such as DeviantArt.</small>
            @endif
...
<hr>
    <h2><i class="fas fa-dragon"></i> Characters</h2>
        <p>Any character set to focus shall receive any stat / level / skill awards. The character must be apart of the activity / the focus character.</p>
...
<hr>
    <h2><i class="fas fa-drumstick-bite"></i> Item Add-Ons</h2>
...
<hr>
    <h2><i class="fas fa-gift"></i> Rewards</h2>
        ...
        @else
            <p>Note that any rewards added here are <u>in addition</u> to the default rewards. If you do not require any additional rewards, you can leave this blank.</p>
        @endif
        @if($isClaim)
