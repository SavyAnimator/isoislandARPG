...
@section('home-title') Critters @endsection

@section('home-content')
{!! breadcrumbs(['Critters' => 'critters']) !!}

<h1>
    Critters
</h1>

<p>These are your critters. Click on a critter to view more details and actions you can perform on it.</p>
                ...
                <div class="row mb-3">
                    @foreach($chunk as $pet)
                        <div class="col-sm-3 col-6 text-center inventory-pet" data-id="{{ $pet->pivot->id }}" data-name="{{ $user->name }}'s {{ $pet->name }}">
                            ...
                            @if($pet->dropData)
                                <a href="{{ url('critters/critter/'.$pet->pivot->id) }}" class="btn btn-primary btn-sm mt-1">Drops</a>
                            @endif
                        </div>
                    @endforeach
                </div>
...
<div class="text-right mb-4">
    <a href="{{ url(Auth::user()->url.'/critter-logs') }}">View logs...</a>
</div>
...
$( document ).ready(function() {
    $('.inventory-stack').on('click', function(e) {
        ...
        loadModal("{{ url('critters') }}/" + $parent.data('id'), $parent.data('name'));
    });
});
