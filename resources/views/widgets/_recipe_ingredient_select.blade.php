        ...
        @if($ingredients)
            ...
            @foreach($ingredients as $ingredient)
                <tr class="ingredient-row" data-row="{{ $row_counter }}">
                    <td>{!! Form::select('ingredient_type['.$row_counter.']', ['Item' => 'Item', 'MultiItem' => 'Multi Item', 'Category' => 'Category', 'MultiCategory' => 'Multi Category', 'Currency' => 'Currency', 'Gear' => 'Accessory', 'Weapon' => 'Equipment', 'MultiWeapon' => 'Multi Equipment'], $ingredient->ingredient_type, ['class' => 'form-control ingredient-type', 'placeholder' => 'Select Ingredient Type']) !!}</td>
                    <td class="ingredient-row-select">
                        @switch($ingredient->ingredient_type)
                            ...
                            @case('Currency')
                                ...
                                @break
                            @case('Gear')
                                {!! Form::select('ingredient_data['.$row_counter.'][]', $gears, $ingredient->data[0], ['class' => 'form-control gear-select selectize', 'placeholder' => 'Select Accessory']) !!}
                                @break
                            @case('Weapon')
                                {!! Form::select('ingredient_data['.$row_counter.'][]', $weapons, $ingredient->data[0], ['class' => 'form-control weapon-select selectize', 'placeholder' => 'Select Equipment']) !!}
                                @break
                            @case('MultiWeapon')
                                <div class="multi-weapon-select-group">
                                    <div class="multi-weapon-list">
                                        @foreach($ingredient->data as $entry)
                                            <div class="mb-2">
                                                {!! Form::select('ingredient_data['.$row_counter.'][]', $weapons, $entry, ['class' => 'form-control weapon-select selectize', 'placeholder' => 'Select Equipment']) !!}
                                                <div class="text-right text-uppercase" style="margin-top: -0.5em;"><a href="#" class="remove-multi-entry-button text-danger {{ count($ingredient->data) > 1 ? '' : 'hide' }}">Remove Equipment</a></div>
                                            </div>
                                        @endforeach
                                    </div>
