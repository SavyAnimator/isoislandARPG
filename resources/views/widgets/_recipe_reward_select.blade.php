    ...
    <tbody id="rewardTableBody">
        @if($rewards)
            @foreach($rewards as $reward)
                <tr class="reward-row">
                    <td>{!! Form::select('rewardable_type[]', ['Item' => 'Item', 'Currency' => 'Currency', 'LootTable' => 'Loot Table', 'Raffle' => 'Raffle', 'Gear' => 'Accessory', 'Weapon' => 'Equipment'], $reward->rewardable_type, [
                    ...
                    <td class="reward-row-select">
                        ...
                        @elseif($reward->rewardable_type == 'Weapon')
                            {!! Form::select('rewardable_id[]', $weapons, $reward->rewardable_id, ['class' => 'form-control weapon-select selectize', 'placeholder' => 'Select Equipment']) !!}
                        @elseif($reward->rewardable_type == 'Gear')
                            {!! Form::select('rewardable_id[]', $gears, $reward->rewardable_id, ['class' => 'form-control gear-select selectize', 'placeholder' => 'Select Accessory']) !!}
                       @endif
                    </td>
