...
<table class="table table-sm" id="lootTable">
    <thead>
        <tr>
            <th width="20%">Reward Type</th>
            <th width="20%">Reward</th>
            <th width="20%">Quantity</th>
            <th width="20%">Tier {!! add_help('Enter in cents the tier it should be locked at. Leave blank for all tiers.') !!}</th>
            <th width="10%">Lock to Tier? {!! add_help('If locked only users exactly matching the tier will receive this reward.') !!}</th>
            <th width="10%"></th>
        </tr>
    </thead>
        ...
        @if($loots)
            @foreach($loots as $loot)
                <tr class="loot-row">
                    ...
                    <td>{!! Form::text('tier[]', $loot->tier, ['class' => 'form-control']) !!}</td>
                    <td>{!! Form::checkbox('lock_tier[]', 1, $loot->lock_tier,['class' => 'form-check-input', 'data-toggle' => 'toggle']) !!}</td>
