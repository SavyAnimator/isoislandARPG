...
@section('content')
<small>{!! breadcrumbs(['Forum' => 'forum' , $thread->commentable->name => 'forum/'.$thread->commentable->id, $thread->title => 'forum/'.$thread->commentable->id.'/'.$thread->id ]) !!}</small>

<h1 class="col-md">
    {!! $thread->displayName !!}
    <a href="{{ url('reports/new?url=') . $thread->threadUrl }}"><i class="fas fa-exclamation-triangle" data-toggle="tooltip" title="Click here to report this thread." style="opacity: 50%;font-size:0.5em; float:right;"></i></a>
</h1>
<hr>
    ...
    @auth
        @can('reply-to-comment', $thread)
            <div class="col-md text-right">
                <button data-toggle="modal" data-target="#reply-modal-{{ $thread->getKey() }}" class="btn px-3 py-2 px-sm-4 py-sm-1 btn-primary text-uppercase"><i class="fas fa-comment"></i><span class="ml-2 d-none d-sm-inline-block">Reply to this topic</span></button>
            </div>
        @endcan
    @endauth
</div>
...
<div class="border mb-2 row no-gutters" style="border-style:double!important; border-width:3px!important;clear:both;">
    <div class="col-md-2 text-center border-md-right border-bottom border-md-bottom-0">
        <img class="mt-2 mw-80" src="/images/avatars/{{ $thread->commenter->avatar }}" style="max-width:80px; max-height:80px; border-radius:50%;" alt="{{ $thread->commenter->name }} Avatar">
        <h5 style="font-size:120%">{!! $thread->commenter->displayName !!}</h5>
    ...
    @foreach($replies as $comment)
        @if(!isset($comment->deleted_at))
            <div class="border mb-2 row no-gutters">
                <div class="col-md-2 text-center border-md-right border-bottom border-md-bottom-0">
                    <img class="mt-2 mw-70" src="/images/avatars/{{ $comment->commenter->avatar }}" style="max-width:70px; max-height:70px; border-radius:50%;" alt="{{ $comment->commenter->name }} Avatar">
                    <h5 style="font-size:110%">{!! $comment->commenter->displayName !!}</h5>
...
@can('reply-to-comment', $thread)
    <div class="card p-3">
        <form method="POST" action="{{ route('comments.reply', $thread->getKey()) }}">
            @csrf
            <h5 class="modal-title">Reply to this topic</h5>
