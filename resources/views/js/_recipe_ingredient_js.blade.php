...
$( document ).ready(function() {
    ...
    var $gearSelect = $('#ingredientRowData').find('.gear-select');
    var $weaponSelect = $('#ingredientRowData').find('.weapon-select');
    var $multiWeaponSelectGroup = $('#ingredientRowData').find('.multi-weapon-select-group');
    var $multiWeaponEntry = $('#ingredientRowData').find('.multi-weapon-entry');
    ...
    attachAddMultiListener($('#ingredientTableBody .add-multi-weapon-button'), 'MultiWeapon');
    ...
    function attachIngredientTypeListener(node) {
        node.on('change', function(e) {
            ...
            else if(val == 'MultiWeapon') {
                $clone = $multiWeaponSelectGroup.clone();
                attachAddMultiListener($clone.find('.add-multi-weapon-button'), val);
                attachRemoveMultiListener($clone.find('.remove-multi-entry-button'));
            }
            ...
            else if(val == 'MultiWeapon') $select = $clone.find('.multi-weapon-select');
            else $select = $clone;
    ...
    function attachAddMultiListener(node, type) {
        ...
        node.on('click', function(e) {
            ...
            else if(type == 'MultiWeapon') {
                $clone = $multiWeaponEntry.clone();
                $(this).parent().find('.multi-weapon-list').append($clone);
                $select = $clone.find('.multi-weapon-select');
            }
            $select.attr('name', 'ingredient_data['+row_num+'][]');
            $select.selectize();
            attachRemoveMultiListener($clone.find('.remove-multi-entry-button'));
