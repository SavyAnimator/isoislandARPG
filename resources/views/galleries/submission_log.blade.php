                                ...
                                {!! Form::open(['url' => 'admin/gallery/edit/'.$submission->id.'/value']) !!}
                                    @if(!$submission->collaborators->count() || $submission->collaborators->where('user_id', $submission->user_id)->first() == null)
                                        <div class="form-group">
                                            ...
                                            {!! Form::number('value[submitted]['.$submission->user->id.']', isset($submission->data['total']) ? round($submission->data['total'] / ($submission->collaborators->count() ? $submission->collaborators->count() : '1')) : 0, ['class' => 'form-control']) !!}
                                        </div>
                                    @endif
                                    @if($submission->collaborators->count())
                                        @foreach($submission->collaborators as $key=>$collaborator)
                                            <div class="form-group">
                                                ...
                                                {!! Form::number('value[collaborator]['.$collaborator->user->id.']', isset($submission->data['total']) ? round($submission->data['total'] / ($submission->collaborators->count() ? $submission->collaborators->count() : '1')) : 0, ['class' => 'form-control']) !!}
                                            </div>
                                        @endforeach
                                    @endif
                                    @if($submission->participants->count())
                                        @foreach($submission->participants as $key=>$participant)
                                            <div class="form-group">
                                                ...
                                                {!! Form::number('value[participant]['.$participant->user->id.']', isset($submission->data['total']) ? ($participant->type == 'Comm' ? round($submission->data['total'] / ($submission->collaborators->count() ? $submission->collaborators->count() : '1')/2) : 0) : 0, ['class' => 'form-control']) !!}
                                            </div>
                                        @endforeach
                                    @endif
                                    <div class="form-group">
