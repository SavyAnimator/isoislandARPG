...
@if(!Auth::check()  || (Auth::check() && Auth::user()->id != $pet->user_id))
    {!! breadcrumbs(['Users' => 'users', $user->name => $user->url, 'Critters' => $user->url . '/critters', $user->name .'\'s '. $pet->pet->name.' '.(isset($pet->drops->dropData->data['drop_name']) ? $pet->drops->dropData->data['drop_name'].'s' : 'drops') => 'critters/critter/'.$pet->id ]) !!}
@else
    {!! breadcrumbs(['Critters' => 'critters', $pet->pet->name.' '.(isset($pet->drops->dropData->data['drop_name']) ? $pet->drops->dropData->data['drop_name'].'s' : 'drops') => 'critters/critter/'.$pet->id ]) !!}
@endif

@if(!$pet->drops->dropData->isActive)
    <div class="alert alert-warning">This critter's drops are currently inactive. Because you are staff, you can see this page anyways.</div>
@endif
    ...
    <div class="col-md-6 text-center">
        ...
        <div class="card card-body mb-4">
            @if($drops->petItem || $drops->variantItem)
                <p>This critter produces these {{ isset($pet->drops->dropData->data['drop_name']) ? strtolower($pet->drops->dropData->data['drop_name']).'s' : 'drops' }}, based on their type of critter and/or variant:</p>
                @if($drops->petItem)
                    <div class="row">
                ...
                @endif
            @else
                <p>This cr {{ isset($pet->drops->dropData->data['drop_name']) ? 'doesn\'t produce any '.strtolower($pet->drops->dropData->data['drop_name']).'s' : 'isn\'t eligible for any drops' }}.</p>
            @endif
        </div>
        ...
        @if($drops->petItem || $drops->variantItem)
            <div class="text-center">
                <p>
                    This critter has {{ $drops->drops_available }} batch{{ $drops->drops_available == 1 ? '' : 'es' }} of {{ isset($pet->drops->dropData->data['drop_name']) ? strtolower($pet->drops->dropData->data['drop_name']) : 'drop' }}s available.<br/>
                    @if(isset($drops->dropData->cap) && $drops->dropData->cap > 0)
                        This critter can manage a maximum of {{ $drops->dropData->cap }} batch{{ $drops->dropData->cap == 1 ? '' : 'es' }} of {{ isset($pet->drops->dropData->data['drop_name']) ? strtolower($pet->drops->dropData->data['drop_name']) : 'drop' }}s at once!
                        @if($drops->drops_available >= $drops->dropData->cap)
                             Until these {{ isset($pet->drops->dropData->data['drop_name']) ? strtolower($pet->drops->dropData->data['drop_name']) : 'drop' }}s are collected, this critter won't produce any more.
                        @else
                             This critter's next {{ isset($pet->drops->dropData->data['drop_name']) ? strtolower($pet->drops->dropData->data['drop_name']) : 'drop' }}(s) will be available to collect {!! pretty_date($drops->next_day) !!}.
                        @endif
                    @else
                        This critter's next {{ isset($pet->drops->dropData->data['drop_name']) ? strtolower($pet->drops->dropData->data['drop_name']) : 'drop' }}(s) will be available to collect {!! pretty_date($drops->next_day) !!}.
                    @endif
            ...
            @if(Auth::check() && Auth::user()->id == $pet->user_id && $drops->drops_available > 0)
                {!! Form::open(['url' => 'critters/critter/'.$pet->id]) !!}
