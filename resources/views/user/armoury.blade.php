...
@section('profile-title') {{ $user->name }}'s Equipment @endsection

@section('profile-content')
{{--{!! breadcrumbs(['Users' => 'users', $user->name => $user->url, 'Equipment' => $user->url . '/equipment']) !!}--}}

<h1>
    {{ $user->name }}'s Accessories & Equipment
</h1>
                ...
                <div class="row mb-3">
                    @foreach($chunk as $gear)
                        <div class="col-sm-3 col-6 text-center inventory-item" data-id="{{ $gear->pivot->id }}" data-name="{{ $user->name }}'s {{ $gear->name }}">
                            <div class="mb-1">
                                <a href="#" class="gear-stack">@if($gear->pivot->has_image)<img src="{{ url('images/data/user-accessory/'.$gear->pivot->id.'-image.png') }}">@else<img src="{{ $gear->imageUrl }}" />@endif</a>
                            </div>
                            <div>
                ...
                <div class="row mb-3">
                    @foreach($chunk as $weapon)
                        <div class="col-sm-3 col-6 text-center inventory-item" data-id="{{ $weapon->pivot->id }}" data-name="{{ $user->name }}'s {{ $weapon->name }}">
                            <div class="mb-1">
                                <a href="#" class="weapon-stack">@if($weapon->pivot->has_image)<img src="{{ url('images/data/user-equipment/'.$weapon->pivot->id.'-image.png') }}">@else<img src="{{ $weapon->imageUrl }}" />@endif</a>
                            </div>
                            <div>
...
$( document ).ready(function() {
    $('.weapon-stack').on('click', function(e) {
        ...
        loadModal("{{ url('equipment') }}/" + $parent.data('id'), $parent.data('name'));
    });
    $('.gear-stack').on('click', function(e) {
        e.preventDefault();
        var $parent = $(this).parent().parent();
        loadModal("{{ url('accessory') }}/" + $parent.data('id'), $parent.data('name'));
    });
});
