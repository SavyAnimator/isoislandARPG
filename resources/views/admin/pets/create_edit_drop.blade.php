...
@section('admin-title') {{ $drop->id ? 'Edit' : 'Create' }} Critter Drop @endsection
...
<h1>{{ $drop->id ? 'Edit' : 'Create' }} Critter Drop Data
    @if($drop->id)
...
<h4>Groups</h4>
<p>
    Every critter of the above selected critter is sorted into a "group" - these groups are used for different item drops, which can be set in this form after the critter drop is initially created.
    These groups can be either assigned at critter creation (either at random or manually after selecting an applicable critter) or may be assigned after critter creation in the critter's "Collect" page, accessed via the pet sidebar on applicable pets.
</p>
