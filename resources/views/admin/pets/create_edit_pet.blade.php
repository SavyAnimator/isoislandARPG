...
{!! breadcrumbs(['Admin Panel' => 'admin', 'Critters' => 'admin/data/pets', ($pet->id ? 'Edit' : 'Create').' Critter' => $pet->id ? 'admin/data/pets/edit/'.$pet->id : 'admin/data/pets/create']) !!}

<h1>{{ $pet->id ? 'Edit' : 'Create' }} Critter
    @if($pet->id)
        <a href="#" class="btn btn-outline-danger float-right delete-pet-button">Delete Critter</a>
    @endif
</h1>

{!! Form::open(['url' => $pet->id ? 'admin/data/pets/edit/'.$pet->id : 'admin/data/pets/create', 'files' => true]) !!}

@if(!$pet->id)<p>You can create variants once the critter is made.<p>@endif
...
<div class="form-group">
    {!! Form::label('Critter Category (Optional)') !!}
...
@if($pet->id)
            ...
            <div id="featureList">
                @foreach($pet->variants as $variant)
                    <div class="form-group d-flex mb-2">
                        {!! Form::text('variant_name[]', $variant->variant_name, ['class' => 'form-control mr-2 feature-select original', 'placeholder' => 'Variant Name']) !!}
    ...
    <div class="feature-row hide mb-2">
        {!! Form::text('variant_name[]', null, ['class' => 'form-control mr-2 feature-select', 'placeholder' => 'Variant Name']) !!}
        {!! Form::file('variant_images[]') !!}
        <a href="#" class="remove-feature btn btn-danger mb-2">×</a>
    </div>
...
$( document ).ready(function() {
    $('.delete-pet-button').on('click', function(e) {
        e.preventDefault();
        loadModal("{{ url('admin/data/pets/delete') }}/{{ $pet->id }}", 'Delete Critter');
    });
