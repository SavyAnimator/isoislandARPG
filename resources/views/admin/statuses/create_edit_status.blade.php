...
<h1>{{ $status->id ? 'Edit' : 'Create' }} Ailment
    @if($status->id)
        <a href="#" class="btn btn-danger float-right delete-status-button">Delete Ailment</a>
    @endif
</h1>
...
$( document ).ready(function() {
    ...
    $('.delete-status-button').on('click', function(e) {
        e.preventDefault();
        loadModal("{{ url('admin/data/status-effects/delete') }}/{{ $status->id }}", 'Delete Ailment');
    });
});
