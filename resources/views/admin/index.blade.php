...
<div class="row">
    @if (Auth::user()->hasPower('manage_submissions'))
      <div class="col-sm-6">
          <div class="card mb-3">
              <div class="card-body">
                  <h5 class="card-title">Activity/Prompt Submissions @if($submissionCount)<span class="badge badge-primary">{{ $submissionCount }}</span>@endif</h5>
                  <p class="card-text">
                      @if($submissionCount)
    ...
    <div class="col-sm-6">
        <div class="card mb-3">
            <div class="card-body">
                <h5 class="card-title">Character Donation Submissions @if($surrenderCount)<span class="badge badge-primary">{{ $surrenderCount }}</span>@endif</h5>
                <p class="card-text">
                    @if($surrenderCount)
                        {{ $surrenderCount }} donation{{ $surrenderCount == 1 ? '' : 's' }} awaiting processing.
                    @else
                        The donation queue is clear. Hooray!
                    @endif
                </p>
                <div class="text-right">
