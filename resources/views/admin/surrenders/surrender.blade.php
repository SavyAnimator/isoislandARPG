...
@section('admin-title') Character Donation (#{{ $surrender->id }}) @endsection

@section('admin-content')
    {!! breadcrumbs(['Admin Panel' => 'admin', 'Submission Queue' => 'admin/surrenders/pending', 'OC Donation (#' . $surrender->id . ')' => $surrender->viewUrl]) !!}

@if($surrender->status == 'Pending')

    <h1>
        Character Donation (#{{ $surrender->id }})
            ...
            <div class="row">
                ...
                <div class="col-sm-4">
                    <a href="{{ $surrender->character->url }}"><h3 class="text-uppercase">{!! $surrender->character->displayname !!}</h3></a>
                        ...
                        <strong>Breed</strong> - {!! $surrender->character->image->subtype_id ? $surrender->character->image->subtype->displayName : 'None' !!}<br>
