...
@else
    <div class="text-center mb-3">
        <div class="mb-1"><a href="{{ $stock->item->idUrl }}"><img src="{{ $stock->item->imageUrl }}" style="max-width:50%" alt="{{ $stock->item->name }}" /></a></div>
        <div><a href="{{ $stock->item->idUrl }}"><strong>{{ $stock->item->name }}</strong></a></div>
        <div><strong>Price: </strong> {!! $stock->currency->display($stock->displayCost) !!}</div>
            ...
            {!! Form::open(['url' => 'shops/buy']) !!}
                ...
                @if($stock->use_user_bank && $stock->use_character_bank)
                    ...
                    <div class="form-group">
                        <div>
                            <label class="h5">{{ Form::radio('bank', 'user' , true, ['class' => 'bank-select mr-1']) }} User Bank
                                &nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;
                ...
                @if($shop->use_coupons)
                <div class="text-success">This shop accepts the following coupons: @foreach($shop->allAllowedCoupons as $coupon) {!! $coupon->displayName !!}{{$loop->last ? '' : ', '}} @endforeach</div>
