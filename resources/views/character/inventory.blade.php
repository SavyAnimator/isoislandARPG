...
<h2 style="display:inline;">{!! $character->displayName !!}'<h2 style="display:inline; text-transform:lowercase;">s</h2><h2 style="display:inline;"> Items</h2>
<hr>
<h3>
            ...
            @foreach($categoryItems->chunk(6) as $chunk)
                <div class="row mb-2">
                    @foreach($chunk as $itemId=>$stack)
                        ...
                        ?>
                        <div class="col-sm-2 col-6 text-center inventory-item" data-id="{{ $stack->first()->pivot->id }}" data-name="{!! $canName && $stackName ? htmlentities($stackNameClean).' [' : null !!}{{ $character->name ? $character->name : $character->slug }}'s {{ $stack->first()->name }}{!! $canName && $stackName ? ']' : null !!}">
                            <div class="mb-1">
