    ...
    <div class="col-md-6 text-center">
        ...
        <div class="card card-body mb-4">
            @if($drops->speciesItem || $drops->subtypeItem)
                @if($drops->speciesItem)
                    <div class="row">
                        <div class="col-md align-self-center">
                            @if($drops->speciesItem->has_image) <img width="70%" src="{{ $drops->speciesItem->imageUrl }}"><br/> @endif
                ...
                @if($drops->subtypeItem)
                    <div class="row">
                        <div class="col-md align-self-center">
                            <h5>{{ $character->image->subtype->name }} (Breed)</h5>
                        </div>
                        <div class="col-md align-self-center">
        ...
        @if(($parent) || ($children->count()))
            ...
            @if($drops->speciesItem || $drops->subtypeItem)
                <div class="text-center">
                    <p>
                        This character has {{ $drops->drops_available }} {{ isset($character->drops->dropData->data['drop_name']) ? strtolower($character->drops->dropData->data['drop_name']) : 'drop' }}{{ $drops->drops_available == 1 ? '' : 's' }} available.<br/>
                        @if(isset($drops->dropData->cap) && $drops->dropData->cap > 0)
                            This character can hold a maximum of {{ $drops->dropData->cap }} {{ isset($character->drops->dropData->data['drop_name']) ? strtolower($character->drops->dropData->data['drop_name']) : 'drop' }}s.
                            ...
                            @else
                                <br>This character's next {{ isset($character->drops->dropData->data['drop_name']) ? strtolower($character->drops->dropData->data['drop_name']) : 'drop' }}(s) will be available to collect {!! pretty_date($drops->next_day) !!}.
                            @endif
                        @else
            ...
            @endif
        @else
        <div class="text-center">{{ $character->fullName }} will not gather care drops nor can you claim them unless an Isomara is companioned. <br>Please submit a <a href="/claims/new">claim</a> with the isomara and companions you'd like to companion together.</div>
        @endif
    </div>
</div>
