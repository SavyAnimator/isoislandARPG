...
class UpdateExtensionTracker extends Command {
    ...
    public function handle() {
        ...
        $extendos = [];
        foreach(glob('config\lorekeeper\ext-tracker\*.php') as $extension) $extendos[basename($extension,'.php')] = include($extension);
        ...
        foreach($extendos as $key => $data)
        {
            ...
            elseif($extension->first()->version != $data['version'])
            {
                ...
                else $this->line('Skipped: '.$key.' / Version: '.$extension->first()->version);
            }
            else $this->line('Skipped: '.$key.' / Version: '.$data['version']);
        }
        ...
        if(app()->runningInConsole()){
            ...
            if(count($missing)){
                ...
                else $this->line("Leaving extension".(count($missing) == 1 ? "" : "s") . " alone.");
