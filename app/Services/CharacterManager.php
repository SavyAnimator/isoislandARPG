    ...
    */
    ...
    public function createCharacter($data, $user, $isMyo = false)
    {
        ...
        try {
            ...
            if(isset($data['parent_id']) && $data['parent_id'])
            {
                ...
                if(!$recipient) $recipient = Character::where('id', $data['parent_id'])->first()->owner_url;
    ...
    private function handleCharacter($data, $isMyo = false)
    {
        try {
            ...
            $characterData['gender_identity'] = 0;
            $characterData['personal_pronouns'] = 0;
            $characterData['chara_height'] = 0;
            $characterData['is_gift_art_allowed'] = 0;
            $characterData['is_gift_writing_allowed'] = 0;
            $characterData['is_trading'] = 0;
            $characterData['is_training'] = 0;
            $characterData['is_explore'] = 0;
            $characterData['is_condensed'] = 0;
    ...
    public function sortCharacters($data, $user)
    {
        ...
        try {
            ...
            foreach($characters as $character)
            {
                ...
                if($folders[$count] == 'Unsettled') $character->folder_id = null;
    ...
    public function updateCharacterProfile($data, $character, $user, $isAdmin = false)
    {
        ...
        try {
            ...
            if(!$isAdmin)
            {
                ...
                $character->is_condensed = isset($data['is_condensed']);
                $character->is_training = isset($data['is_training']) && $data['is_training'] <= 2 ? $data['is_training'] : 0;
                $character->is_explore = isset($data['is_explore']) && $data['is_explore'] <= 2 ? $data['is_explore'] : 0;
                $character->gender_identity = isset($data['gender_identity']) && $data['gender_identity'] <= 12 ? $data['gender_identity'] : 0;
                $character->personal_pronouns = isset($data['personal_pronouns']) && $data['personal_pronouns'] <= 6 ? $data['personal_pronouns'] : 0;
                $character->chara_height = isset($data['chara_height']) && $data['chara_height'] <= 59 ? $data['chara_height'] : 0;
                $character->save();
            }
            else {
            }
