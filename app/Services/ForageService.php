    ...
    */
    ...
    public function initForage($id, $user)
    {
        ...
        try {
            ...
            $user->foraging->distribute_at = carbon::now()->addSeconds(Config::get('lorekeeper.foraging.forage_time'));
    ...
    public function claimReward($user)
    {
        ...
        try {
            ...
            $distribute = $user->foraging->foraged_at->addSeconds(Config::get('lorekeeper.foraging.forage_time'));
    ...
    private function getRewardsString($rewards, $character_name = null)
    {
        ...
        foreach($rewards as $assetType)
        {
            if(isset($assetType))
            {
                ...
                {
                    array_push($result_elements, "<center> <img src='".($asset['asset']->image_url ? $asset['asset']->image_url : $asset['asset']->currencyImageUrl)."' style='max-height: 100px;'>"."<br>".$asset['quantity']." ".$asset['asset']->name.(class_basename($asset['asset']) == 'Raffle' ? ' (Raffle Ticket)' : '')."<br>");
                }
            }
        }
        return $results.implode('<br>', $result_elements)."</center>";
    }
