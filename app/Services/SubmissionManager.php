    ...
    */
    ...
    public function createSubmission($data, $user, $isClaim = false)
    {
        ...
        try {
            ...
            $currencyIds = []; $itemIds = []; $tableIds = []; $featureIds = []; $statusIds = [];
            ...
            foreach($characters as $key => $c)
            {
                ...
                $assets = $this->processRewards($data + ['character_id' => $c->id, 'currencies' => $currencies, 'items' => $items, 'tables' => $tables, 'features' => $features, 'statuses' => $statuses], true);
    ...
    public function approveSubmission($data, $user)
    {
        ...
        try {
            ...
            $currencyIds = []; $itemIds = []; $tableIds = []; $awardIds = []; $featureIds = []; $statusIds = [];
            ...
            array_unique($currencyIds);            array_unique($itemIds);            array_unique($tableIds);          array_unique($awardIds);            array_unique($featureIds);            array_unique($statusIds);
            $currencies = Currency::whereIn('id', $currencyIds)->where('is_character_owned', 1)->get()->keyBy('id');
            $items = Item::whereIn('id', $itemIds)->get()->keyBy('id');
            $tables = LootTable::whereIn('id', $tableIds)->get()->keyBy('id');
            ...
            foreach($characters as $key => $c)
            {
                ...
                $assets = $this->processRewards($data + ['character_id' => $c->id, 'currencies' => $currencies, 'items' => $items, 'tables' => $tables, 'awards' => $awards, 'features' => $features, 'statuses' => $statuses], true);
                ...
                if(isset($data['character_is_focus']) && $data['character_is_focus'][$c->id] && $submission->prompt_id) {
                    ...
                    if($submission->prompt->expreward) {
                        ...
                        if($submission->prompt->expreward->chara_points || isset($data['bonus_points']))
                        {
                            ...
                            if(!$statLog->creditStat(null, $c, $promptLogType, $levelData, $quantity)) throw new \Exception('Could not grant stat points');
                        }
                    }
                }
            }
