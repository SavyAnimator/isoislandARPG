    ...
    public function act($stacks, $user, $data)
    {
        ...
        try {
            foreach($stacks as $key=>$stack) {
                ...
                if((new InventoryManager)->debitStack($stack->user, 'Cracked Open', ['data' => ''], $stack, $data['quantities'][$key])) {

                    for($q=0; $q<$data['quantities'][$key]; $q++) {
                        // Distribute user rewards
                        if(!$rewards = fillUserAssets(parseAssetData($stack->award->tag('box')->data), $user, $user, 'Openable Rewards', [
                            'data' => 'Received rewards from opening '.$stack->award->name
                        ])) throw new \Exception("Failed to crack open.");
    ...
    private function getBoxRewardsString($rewards)
    {
        ...
        foreach($rewards as $assetType)
        {
            if(isset($assetType))
            {
                ...
                {
                    array_push($result_elements, "<img src='".($asset['asset']->image_url ? $asset['asset']->image_url : $asset['asset']->currencyImageUrl)."' style='max-height: 20px;'>"." ".$asset['quantity']." ".$asset['asset']->name);
                }
            }
        }
