...
class PetController extends Controller
{
    ...
    */
    ...
    public function postVariant(Request $request, PetManager $service, $id, $isStaff = false)
    {
        ...
        if(!$request->input('stack_id') && !Auth::user()->isStaff) {  flash('No variant brush selected.')->error();
            return redirect()->back();
        }
        ...
        if($request->input('stack_id')) {
            ...
            if(!$invman->debitStack($pet->user, 'Used to change critter variant', ['data' => 'Used to change '.$pet->pet->name.' variant'], $item, 1)) {
                flash('Could not debit splice.')->error();
                return redirect()->back();
            }
        }
        if($service->editVariants($request->input('variant_id'), $pet, $request->input('stack_id'), $request->input('is_staff'))) {
