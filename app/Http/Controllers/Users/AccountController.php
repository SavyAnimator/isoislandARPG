    ...
    */
    ...
    public function postLinks(Request $request)
    {
        Auth::user()->profile->update([
            ...
            'devian' => $request->get('devian'),
            'house' => $request->get('house'),
            'insta' => $request->get('insta'),
            'tumb' => $request->get('tumb'),
            'twitt' => $request->get('twitt'),
            'bluesk' => $request->get('bluesk'),
            'youtu' => $request->get('youtu'),
            'newgr' => $request->get('newgr'),
            'artsta' => $request->get('artsta'),
        ]);
        flash('Profile updated successfully.')->success();
        return redirect()->back();
    }
     ...
     */
     ...
     public function postOnline(Request $request, UserService $service)
     {
         if($service->updateONL($request->input('online_setting'), Auth::user())) {
             flash('Setting updated successfully.')->success();
         } else {
             foreach ($service->errors()->getMessages()['error'] as $error) flash($error)->error();
         }
