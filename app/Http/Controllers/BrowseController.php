...
class BrowseController extends Controller
{
    ...
    */
    ...
    public function getCharacters(Request $request)
    {
        ...
        if($request->get('is_training')) switch($request->get('is_training')) {
            case 1:
                $query->where('is_training', 1);
            break;
            case 2:
                $query->where('is_training', 2);
            break;
            case 3:
                $query->where('is_training', '>=', 1);
            break;
        }
        if($request->get('is_explore')) switch($request->get('is_explore')) {
            case 1:
                $query->where('is_explore', 1);
            break;
            case 2:
                $query->where('is_explore', 2);
            break;
            case 3:
                $query->where('is_explore', '>=', 1);
            break;
        ...
        return view('browse.masterlist', [
            'isMyo' => false,
            'characters' => $query->paginate(60)->appends($request->query()),
            ...
            'userOptions' => User::query()->orderBy('name')->where('is_banned', 0)->pluck('name', 'id')->toArray(),
        ]);
    }
