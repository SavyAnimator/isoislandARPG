...
class PetController extends Controller
{
    /*
    |--------------------------------------------------------------------------
    | Admin / Critter Controller
    |--------------------------------------------------------------------------
    |
    | Handles creation/editing of critter categories and pets/critters.
    |
    */
    ...
    **********************************************************************************************/
    ...
    /**
     * Shows the critter category index.
     *
    ...
    /**
     * Shows the create critter category page.
     *
     * @return \Illuminate\Contracts\Support\Renderable
     */
    ...
    /**
     * Shows the edit critter category page.
     *
     * @param  int  $id
     * @return \Illuminate\Contracts\Support\Renderable
     */
    ...
    /**
     * Creates or edits an critter category.
     *
    ...
    **********************************************************************************************/
    ...
    public function postCreateEditPet(Request $request, PetService $service, $id = null)
    {
        ...
        if($id && $service->updatePet(Pet::find($id), $data, Auth::user())) {
            flash('Critter updated successfully.')->success();
        }
        else if (!$id && $pet = $service->createPet($data, Auth::user())) {
            flash('Critter created successfully.')->success();
    ...
    /**
     * Gets the critter deletion modal.
     *
     * @param  int  $id
     * @return \Illuminate\Contracts\Support\Renderable
     */
    ...
    public function postDeletePet(Request $request, PetService $service, $id)
    {
        if($id && $service->deletePet(Pet::find($id))) {
            flash('Critter deleted successfully.')->success();
        }
        else {
            foreach($service->errors()->getMessages()['error'] as $error) flash($error)->error();
        }
 ...
 /**********************************************************************************************
    ...
    **********************************************************************************************/
    ...
    {
        if($id && $service->editVariants($request->only(['variant_name', 'variant_images']), Pet::find($id))) {
    ...
    public function postEditPetDrop(Request $request, $id)
    {
        ...
        if ($drops->update(['parameters' => $request['parameters'], 'drops_available' => $request['drops_available']])) {
            flash('Critter drops updated successfully.')->success();
        }
        else {
            flash('Failed to update critter drops.')->error();
        }
    ...
    /**
     * Shows the create critter drop data page.
     *
     * @return \Illuminate\Contracts\Support\Renderable
     */
    ...
    /**
     * Shows the edit critter drop data page.
     *
     * @param  int  $id
     * @return \Illuminate\Contracts\Support\Renderable
     */
    ...
    /**
     * Creates or edits critter drop data.
     *
     * @param  \Illuminate\Http\Request           $request
    ...
    public function postCreateEditDrop(Request $request, PetDropService $service, $id = null)
    {
        ...
        if($id && $service->updatePetDrop(PetDropData::find($id), $data, Auth::user())) {
            flash('Critter drop updated successfully.')->success();
        }
        else if (!$id && $drop = $service->createPetDrop($data, Auth::user())) {
            flash('Critter drop created successfully.')->success();
    ...
    /**
     * Gets the critter drop data deletion modal.
     *
     * @param  int  $id
     * @return \Illuminate\Contracts\Support\Renderable
     */
