...
class User extends Authenticatable implements MustVerifyEmail {
     ...
     */
    protected $fillable = [
        'name', 'alias', 'rank_id', 'email', 'email_verified_at', 'password', 'is_news_unread', 'is_banned', 'has_alias', 'avatar', 'is_sales_unread', 'theme_id', 'decorator_theme_id', 'birthday', 'disc', 'devian', 'house', 'insta', 'tumb', 'bluesk','twitt', 'youtu', 'newgr', 'artsta', 'is_deactivated', 'deactivater_id', 'is_polls_unread', 'last_seen', 'referred_by',
    ];
    ...
    {
        return $this->hasOne('App\Models\User\UserPatreon', 'user_id');
    }
    ...
    {
        return $this->belongsToMany('App\Models\Item\Item', 'user_items')->withPivot('count', 'data', 'updated_at', 'id', 'garden_count')->whereNull('user_items.deleted_at');
    }
   ...
   public function awards()
   {
    ...
    /**
     * gets all the user's garden plots
     */
    public function gardenPlots()
    {
        return $this->hasMany('App\Models\User\UserGardenPlot');
    }
    ...
    public function getCommentDisplayNameAttribute() {
        return '<small><a href="' . $this->url . '" "' . ($this->rank->color ? 'style="background-color: #' . $this->rank->color . '!important;color:#000!important;"' : '') . '><i class="' . ($this->rank->icon ? $this->rank->icon : 'fas fa-user') . ' mr-1" style="opacity: 50%;"></i>' . $this->name . '</a></small>';
    }
    ...
    public function isOnline()
    {
      ...
      if($onlineStatus) $result = '<i class="fas fa-circle text-ilgreen mr-2" data-toggle="tooltip" title="This user is online."></i>';
